...
class Netfilter extends BaseController
{
    ...
    public function levelPolicyCheck():void{
        ...
        $range = [
            0 => "-m iprange --src-range 192.168.1.1-192.168.1.50",
            1 => "-m iprange --src-range 192.168.1.51-192.168.1.100",
            2 => "-m iprange --src-range 192.168.1.101-192.168.1.150",
            3 => "-m iprange --src-range 192.168.1.151-192.168.1.200",
            4 => "-m iprange --src-range 192.168.1.201-192.168.1.250"
        ];
        ...
        for($line = 1 ; $line < 6; $line++)
        {
            ...
            exec($cmd, $ruleInput);
            if(empty($ruleInput ) || !strpos($ruleInput[0], $range[$line-1]))
                exec('./init.sh');

            $cmd = "sudo iptables -S FORWARD ".$line;
            exec($cmd, $ruleForward);
            if(empty($ruleForward) || !strpos($ruleForward[0], $range[$line-1]))
                exec('./init.sh');

            $cmd = "sudo iptables -S OUTPUT ".$line;
            exec($cmd, $ruleOutput);
            if(empty($ruleOutput) || !strpos($ruleOutput[0], $range[$line-1]))
                exec('./init.sh');
    ...
    public function changePoliceL1(): void {
        $input = $this->request->getVar("input");
        if(isset($input)) {
            $command_input = "sudo iptables -R INPUT 1 -m iprange --src-range 192.168.1.1-192.168.1.50 -j " . $input;
            exec($command_input);
        }

        $forward = $this->request->getVar("forward");
        if(isset($forward)) {
            $command_forward = "sudo iptables -R FORWARD 1 -m iprange --src-range 192.168.1.1-192.168.1.50 -j " . $forward;
            exec($command_forward);
        }

        $output = $this->request->getVar("output");
        if(isset($output)) {
            $command_output = "sudo iptables -R OUTPUT 1 -m iprange --src-range 192.168.1.1-192.168.1.50 -j " . $output;
            exec($command_output);
        }
    }
    ...
    public function changePoliceL2(): void {
        $input = $this->request->getVar("input");
        if(isset($input)) {
            $command_input = "sudo iptables -R INPUT 2 -m iprange --src-range 192.168.1.51-192.168.1.100 -j " . $input;
            exec($command_input);
        }

        $forward = $this->request->getVar("forward");
        if(isset($forward)) {
            $command_forward = "sudo iptables -R FORWARD 2 -m iprange --src-range 192.168.1.51-192.168.1.100 -j " . $forward;
            exec($command_forward);
        }

        $output = $this->request->getVar("output");
        if(isset($output)) {
            $command_output = "sudo iptables -R OUTPUT 2 -m iprange --src-range 192.168.1.51-192.168.1.100 -j " . $output;
            exec($command_output);
        }
    }
    ...
    public function changePoliceL3(): void {
        $input = $this->request->getVar("input");
        if(isset($input)) {
            $command_input = "sudo iptables -R INPUT 3 -m iprange --src-range 192.168.1.101-192.168.1.150 -j " . $input;
            exec($command_input);
        }

        $forward = $this->request->getVar("forward");
        if(isset($forward)) {
            $command_forward = "sudo iptables -R FORWARD 3 -m iprange --src-range 192.168.1.101-192.168.1.150 -j " . $forward;
            exec($command_forward);
        }

        $output = $this->request->getVar("output");
        if(isset($output)) {
            $command_output = "sudo iptables -R OUTPUT 3 -m iprange --src-range 192.168.1.101-192.168.1.150 -j " . $output;
            exec($command_output);
        }
    }
    ...
    public function changePoliceM1(): void {
        $input = $this->request->getVar("input");
        if(isset($input)) {
            $command_input = "sudo iptables -R INPUT 4 -m iprange --src-range 192.168.1.151-192.168.1.200 -j " . $input;
            exec($command_input);
        }

        $forward = $this->request->getVar("forward");
        if(isset($forward)) {
            $command_forward = "sudo iptables -R FORWARD 4 -m iprange --src-range 192.168.1.151-192.168.1.200 -j " . $forward;
            exec($command_forward);
        }

        $output = $this->request->getVar("output");
        if(isset($output)) {
            $command_output = "sudo iptables -R OUTPUT 4 -m iprange --src-range 192.168.1.151-192.168.1.200 -j " . $output;
            exec($command_output);
        }
    }
    ...
    public function changePoliceM2(): void {
        $input = $this->request->getVar("input");
        if(isset($input)) {
            $command_input = "sudo iptables -R INPUT 5 -m iprange --src-range 192.201.1.201-192.168.1.250 -j " . $input;
            exec($command_input);
        }

        $forward = $this->request->getVar("forward");
        if(isset($forward)) {
            $command_forward = "sudo iptables -R FORWARD 5 -m iprange --src-range 192.201.1.201-192.168.1.250 -j " . $forward;
            exec($command_forward);
        }

        $output = $this->request->getVar("output");
        if(isset($output)) {
            $command_output = "sudo iptables -R OUTPUT 5 -m iprange --src-range 192.201.1.201-192.168.1.250 -j " . $output;
            exec($command_output);
        }
    }
